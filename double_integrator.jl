...
function find_tau_star(x0::Tuple2f, v0::Tuple2f, x1::Tuple2f, v1::Tuple2f)::Tuple2f
    x01 = x1 - x0
    v01 = v1 - v0
    p = -4.0*(dot(v0, v0)+dot(v1, v1)+dot(v0, v1))
    q = 24.0*dot(v0+v1, x01)
    r = -36.0*dot(x01, x01)

    # f(t) correspoding to cost(t)
    f = t->t + dot(v01, 4.0*v01/t-6(-v0+x01)/t^2)+dot(-6.0*v01/t^2+12.0(x01-v0*t)/t^3, -v0*t+x01)
    df = t->t*t*t*t+p*t*t+q*t+r # df(t)/dt
    ddf = t->4.0*t*t*t+2*p*t+q # ddf(t)/dt^2
